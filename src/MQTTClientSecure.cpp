...
// default constructor
MQTTClientSecure::MQTTClientSecure(
    const short mqttServerPort,
    const char *mqttClientName) : MQTTClientSecure(nullptr, mqttServerPort, mqttClientName)
{
}
...
MQTTClientSecure::MQTTClientSecure(
    ...
    const short mqttServerPort,
    const char *mqttClientName) : MQTTClientSecure(NULL, NULL, mqttServerIp, NULL, NULL, mqttClientName, mqttServerPort)
{
}
...
MQTTClientSecure::MQTTClientSecure(
    ...
    const char *mqttClientName) : MQTTClientSecure(NULL, NULL, mqttServerIp, mqttUsername, mqttPassword, mqttClientName, mqttServerPort)
{
}
...
MQTTClientSecure::MQTTClientSecure(
    ...
    const short mqttServerPort) : MQTTClientSecure(wifiSsid, wifiPassword, mqttServerIp, NULL, NULL, mqttClientName, mqttServerPort)
{
}
...
void MQTTClientSecure::enableDebuggingMessages(const bool enabled)
{
  _enableSerialLogs = enabled;
}

void MQTTClientSecure::enableMQTTPersistence()
{
  _mqttCleanSession = false;
}

void MQTTClientSecure::enableLastWillMessage(const char *topic, const char *message, const bool retain)
{
...
bool MQTTClientSecure::handleWiFi()
{
...
bool MQTTClientSecure::handleMQTT()
{
...
void MQTTClientSecure::onWiFiConnectionEstablished()
{
  if (_enableSerialLogs)
    Serial.printf("WiFi: Connected (%fs), ip : %s \n", millis() / 1000.0, WiFi.localIP().toString().c_str());
}

void MQTTClientSecure::onWiFiConnectionLost()
{
  if (_enableSerialLogs)
...
void MQTTClientSecure::onMQTTConnectionEstablished()
{
  _connectionEstablishedCount++;
  _connectionEstablishedCallback();
}

void MQTTClientSecure::onMQTTConnectionLost()
{
  if (_enableSerialLogs)
  {
...
bool MQTTClientSecure::setMaxPacketSize(const uint16_t size)
{
...
bool MQTTClientSecure::publish(const String &topic, const String &payload, bool retain)
{
...
bool MQTTClientSecure::subscribe(const String &topic, MessageReceivedCallback messageReceivedCallback, uint8_t qos)
{
...
bool MQTTClientSecure::subscribe(const String &topic, MessageReceivedCallbackWithTopic messageReceivedCallback, uint8_t qos)
{
...
bool MQTTClientSecure::unsubscribe(const String &topic)
{
...
void MQTTClientSecure::setKeepAlive(uint16_t keepAliveSeconds)
{
  _mqttClient.setKeepAlive(keepAliveSeconds);
}

void MQTTClientSecure::setWifiCredentials(const char *wifiSsid, const char *wifiPassword)
{
  _wifiSsid = wifiSsid;
  _wifiPassword = wifiPassword;
  _handleWiFi = true;
...
void MQTTClientSecure::executeDelayed(const unsigned long delay, DelayedExecutionCallback callback)
{
...
void MQTTClientSecure::connectToWifi()
{
  WiFi.mode(WIFI_STA);
#ifdef ESP32
...
bool MQTTClientSecure::connectToMqttBroker()
{
  bool success = false;
...
void MQTTClientSecure::processDelayedExecutionRequests()
{
 ...
 */
bool MQTTClientSecure::mqttTopicMatch(const String &topic1, const String &topic2)
{
  int i = 0;
...
void MQTTClientSecure::mqttMessageReceivedCallback(char *topic, uint8_t *payload, unsigned int length)
{
...
void MQTTClientSecure::setCACert(const char *cert)
{
  _wifiClient.setCACert(cert);
}

void MQTTClientSecure::setInsecure()
{
